...
model_size = st.sidebar.selectbox("YOLO model", ["yolov8n.pt", "yolov8s.pt", "yolov8m.pt"], index=0)
conf_thres = st.sidebar.slider("Confidence threshold", 0.2, 0.9, 0.45)
use_gpu = st.sidebar.checkbox("Use GPU (if available)", value=False)
source_type = st.sidebar.selectbox("Source", ["Upload video", "Webcam (0)"])
fps_limit = st.sidebar.slider("Process FPS", 1, 30, 6)

st.title("🐾 Human + Animal Counter (YOLOv8 + Streamlit)")
st.markdown("Counts humans and common animals (COCO pretrained).")
...
if (uploaded and start) or (source_type == "Webcam (0)" and start):
    ...
    while cap.isOpened():
        ...
        for didx, db in enumerate(dets):
            ...
            cv2.putText(annotated, label, (x1, y1-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2)
